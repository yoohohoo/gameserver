...
func InsertUser(u *model.User) (int64, error) {
	...
	result, err := database.Exec("insert into user(algo, hash, role, coin, salt, priv_keyey,pub_key, status, is_guest) values (?,?,?,?,?,?,?,?, ?)",
		u.Algo, u.Hash, u.Role, u.Coin, u.Salt, u.PrivKeyey, u.PubKey, u.Status, u.IsGuest)
	if err != nil {
		return 0, err
	}
...
func QueryUserInfo(id int64) (*protocol.UserStatsInfo, error) {
	...
	usi := &protocol.UserStatsInfo{
		...
		Uid:            u.Id,
		Name:           ta.ThirdName,
		RegisterAt:     r.CreateAt.Unix(),
		RegisterIP:     r.Remote,
		LastestLoginAt: l.LoginAt,
		LastestLoginIP: l.Remote,
		RemainCard:     int(u.Coin),
		TotalMatch:     0,
	}
